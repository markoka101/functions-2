...
// CODE HERE
const evenNumbers = mixedNumbers.filter(elem => elem % 2 === 0);// = mixedNumbers.filter(/* Provide Your Callback Here */)
...
// CODE HERE
const postTaxPrices = prices.map(elem => (elem * 1.07).toFixed(2)); // = prices.map(/* Provide Your Callback Here );
...
// CODE HERE
const totalPopulation = populations.reduce((aelem, celem) => aelem + celem ) //  = populations.reduce(/* Provide Your Callback Here */)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(elem => elem['CP'] > 200);// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
// CODE HERE
const totalOrder = orders.map(elem => (elem['price']*(elem['tax']+1)).toFixed(2));
...
// CODE HERE
const bobsTotal = purchases.filter(elem => elem['owner'] === 'Bob').map(elem => elem['price']).reduce((a, c) => a + c);
